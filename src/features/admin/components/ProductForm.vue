...
const validationSchema = toFormValidator(
  z.object({
    title: z
      ...
      .max(10, 'Le titre doit faire moins de 10  caractères'),
    image: z.string(required),
    price: z
      .number(required)
      .min(0, { message: 'Le prix doit être supérieur à 0' })
      .min(15000, { message: 'Le prix doit être inférieur à 15 000€' }),
    description: z
      .string(required)
...
<template>
  <div class="card">
    ...
    <form @submit="trysubmit">
      ...
      <button class="btn-primary" :disabled="isSubmitting">Sauvegarder</button>
    </form>
  </div>
</template>
